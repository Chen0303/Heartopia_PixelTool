      ...
      <div class="content">
        <div class="preview-wrapper" ref="wrapperRef">
          <div class="zoom-controls">
            <el-icon @click.stop="zoomIn"><ZoomIn /></el-icon>
            <el-icon @click.stop="zoomOut"><ZoomOut /></el-icon>
          </div>
          <canvas
            ref="previewCanvas"
            @mousemove="
            ...
            >
              ...
              <div class="palette-main" v-if="group[0]">
                <div
                  ...
                  :class="{
                    highlighted:
                      group[0].hex.toLowerCase() ===
                      highlightedHex.toLowerCase(),
                  }"
                  :style="{ background: group[0].hex }"
                >
              ...
              <div class="palette-sub" v-if="group.length > 1">
                <div
                  ...
                  :class="{
                    highlighted:
                      color.hex.toLowerCase() === highlightedHex.toLowerCase(),
                  }"
...
// Props
const props = defineProps<{ sourceCanvas: HTMLCanvasElement | null }>();
...
function drawPreview() {
  ...
  ctx.drawImage(props.sourceCanvas, 0, 0, srcW, srcH, offsetX, offsetY, drawW, drawH);
}
...
// Zoom
function zoomIn() {
  zoom.value = Math.min(zoom.value + 0.5, MAX_ZOOM);
  adjustOffset();
  drawPreview();
}

function zoomOut() {
  zoom.value = Math.max(zoom.value - 0.5, MIN_ZOOM);
  adjustOffset();
  drawPreview();
}
